...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (action === 'Candidate') {
      apiUrl = 'http://localhost:5000/api/candidate/createuser';
    } else if (action === 'Employer') {
      apiUrl = 'http://localhost:5000/api/employer/createuser';
    }
    ...
    if(json.success){
      ...
      if (action === 'Candidate') {
        navigate('/login'); // Redirect candidates to the "Jobs" page
      } else if (action === 'Employer') {
        navigate('/login'); // Redirect employers to the "Employer" page
      }
    }
  }
        ...
        <div className='head'>
            ...
            <div className='inputs'>
              <div className='input'>
                ...
                <input type='text' value={credentials.name} name='name'  onChange={onChange} placeholder='Name'/>
              </div>
              <div className='input'>
